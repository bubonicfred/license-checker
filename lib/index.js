...
const { lstat, readdir, readFile, fileExists } = require("./utils");
...
var flatten = async ({ deps: json, data, color: colorize, unknown, customFormat, production, development, basePath, _args }) => {
  ...
  if ((production && json.extraneous) || (development && !json.extraneous && !json.root)) {
    return data;
  }
  ...
  const include = (property) => { return (customFormat === undefined || customFormat[property] !== false); };

  if (include("repository") && json.repository && typeof json.repository === "object") {
    const { url: repoUrl } = json.repository;
    moduleInfo.repository = repoUrl.replace("git+ssh://git@", "git://")
  ...
  if (json.author && typeof json.author === "object") {
    const { name, email, url } = json.author;
    if (include("publisher") && name) {
      moduleInfo.publisher = name;
    }
  ...
  if (customFormat) {
    Object.keys(customFormat).forEach(function forEachCallback(item) {
      if (include(item) && json[item]) {
        ...
        moduleInfo[item] = customFormat[item];
      }
    });
  }
  ...
  files.forEach((filename, fileIndex) => {
    ...
    if (include("licenseFile")) {
      moduleInfo.licenseFile = basePath ? path.relative(basePath, licenseFile) : licenseFile;
    }

    if (include("licenseText") && customFormat) {
      if (!content) {
        content = fs.readFileSync(licenseFile, { encoding: "utf8" });
      }
      /* istanbul ignore else */
      moduleInfo.licenseText = _args && !_args.csv ? content.trim() : content.replace(/"/g, "'").replace(/\r?\n|\r/g, " ").trim();
    }

    if (!(include("copyright") && customFormat)) {
      return;
    }
    if (!content) {
      content = fs.readFileSync(licenseFile, { encoding: "utf8" });
  ...
  noticeFiles.forEach((filename) => {
    ...
    if (fs.lstatSync(file).isFile()) {
      moduleInfo.noticeFile = basePath ? path.relative(basePath, file) : file;
    }
  });
  ...
  if (json.dependencies) {
    Object.keys(json.dependencies).forEach((name) => {
      const { name: childName, version: childVersion } = json.dependencies[name];
      const dependencyId = `${childName}@${childVersion}`;

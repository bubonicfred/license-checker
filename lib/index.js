...
var flatten = (options) => {
  ...
  const include = (property) => (options.customFormat === undefined ||
  ...
  if (include("repository") && json.repository &&
    ...
    moduleInfo.repository =
        moduleInfo.repository.replace('git://github.com', 'https://github.com');
    moduleInfo.repository =
        moduleInfo.repository.replace('git@github.com:', 'https://github.com/');
  ...
  if (json.path && (!json.readme || json.readme.toLowerCase().includes(
  ...
  files.forEach((filename, index) => {
    ...
    if (include("licenseText") && options.customFormat) {
      ...
      moduleInfo.licenseText =
          options._args && !options._args.csv
              ? content.trim()
    ...
    const linesWithCopyright =
        content.replace(/\r\n/g, '\n')
            ...
            .filter(function selectCopyRightStatements(value) {
              return value.startsWith('opyright',
                                      1) && // include copyright statements
                     !value.startsWith(
                         'opyright notice',
...
exports.init = function(options, callback) {
  ...
  read(options.start, opts, (err, json) => {
    ...
    Object.keys(data).sort().forEach((item) => {
      ...
      if (options.unknown &&
          (data[item].licenses && data[item].licenses !== UNKNOWN) &&
          data[item].licenses.includes('*')) {
        /*istanbul ignore if*/
        data[item].licenses = colorizeString(UNKNOWN);
      }
      /*istanbul ignore else*/
      if (data[item]) {
        if (options.onlyunknown) {
          if (data[item].licenses.includes('*') ||
              data[item].licenses.includes(UNKNOWN)) {
      ...
      Object.keys(sorted).forEach((item) => {
        ...
        if (licenses) {
          ...
          licenses.forEach((license) => {
            /*istanbul ignore if - just for protection*/
            if (license.includes(UNKNOWN)) { // necessary due to colorization
            ...
            if (invalidSPDXLicenses.includes(license)) {
              licenseMatch = true;
    ...
    Object.keys(restricted).forEach((item) => {
      if (toCheckforFailOn.length > 0 &&
          toCheckforFailOn.includes(restricted[item].licenses)) {
      ...
      if (!good) {
        console.error(`Package "${item}" is licensed under "${
            restricted[item]
...
exports.print = (sorted) => { console.log(exports.asTree(sorted)); };
...
exports.asSummary = (sorted) => {
  ...
  Object.keys(licenseCountObj).forEach((license) => {
    licenceCountArray.push({license, count : licenseCountObj[license]});
  });

  /*istanbul ignore next*/
  licenceCountArray.sort((a, b) => b.count - a.count);
...
exports.asCSV = (sorted, customFormat, csvComponentPrefix) => {
  ...
  if (customFormat && Object.keys(customFormat).length > 0) {
    ...
    Object.keys(customFormat)
        .forEach(function forEachCallback(item) { textArr.push(`"${item}"`); });
  } else {
    textArr = [];
    /*istanbul ignore next*/
    if (csvComponentPrefix) {
    ...
    ['"module name"', '"license"', '"repository"'].forEach(
  ...
  Object.keys(sorted).forEach((key) => {
    ...
    if (customFormat && Object.keys(customFormat).length > 0) {
      ...
      lineArr.push([
        `"${key}"`, `"${module.licenses || ''}"`, `"${module.repository || ''}"`
      ]);
    }
    line = lineArr.join(',');
    text.push(line);

...
exports.init = function(options, callback) {
    ...
    read(options.start, opts, (err, json) => {
        ...
        const sorted = {};
        let filtered = {};
        const exclude = options.exclude && options.exclude.match(/([^\\\][^,]|\\,)+/g).map((license) => {return license.replace(/\\,/g, ',').replace(/^\s+|\s+$/g, '');});
        let inputError = null;
...
exports.asTree = (sorted) => {return treeify.asTree(sorted, true);};

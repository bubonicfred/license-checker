...
import read from 'read-installed';
...
var flatten = function(options) {
  var moduleInfo = {licenses : UNKNOWN}, json = options.deps,
      data = options.data, key = json.name + '@' + json.version,
      colorize = options.color, unknown = options.unknown, readmeFile,
  ...
  if ((options.production && json.extraneous) ||
  ...
  var include = function(property) {
    return (options.customFormat === undefined ||
  ...
  if (include("repository") && json.repository) {
    ...
    if (typeof json.repository === 'object' &&
      ...
      moduleInfo.repository =
          json.repository.url.replace('git+ssh://git@', 'git://');
  ...
  if (json.path && (!json.readme || json.readme.toLowerCase().indexOf(
                                        'no readme data found') > -1)) {
    readmeFile = join(json.path, 'README.md');
    /*istanbul ignore if*/
    if (existsSync(readmeFile)) {
      json.readme = readFileSync(readmeFile, 'utf8').toString();
    }
  }
  ...
  if (json.path && existsSync(json.path)) {
    dirFiles = readdirSync(json.path);
    files = licenseFiles(dirFiles);

    noticeFiles = dirFiles.filter(function(filename) {
      filename = filename.toUpperCase();
      var name = basename(filename).replace(extname(filename), '');
      return name === 'NOTICE';
    });
  }

  files.forEach(function(filename, index) {
    licenseFile = join(json.path, filename);
    // Checking that the file is in fact a normal file and not a directory for
    // example.
    /*istanbul ignore else*/
    if (lstatSync(licenseFile).isFile()) {
      var content;
      if (!moduleInfo.licenses || moduleInfo.licenses.indexOf(UNKNOWN) > -1 ||
        ...
        content = readFileSync(licenseFile, {encoding : 'utf8'});
      ...
      if (index === 0) {
        ...
        if (include("licenseFile")) {
          moduleInfo.licenseFile = options.basePath
                                       ? relative(options.basePath, licenseFile)
                                       : licenseFile;
        }
        ...
        if (include('copyright') && options.customFormat) {
          ...
          var linesWithCopyright =
              content.replace(/\r\n/g, '\n')
                  .split('\n\n')
                  .filter(function selectCopyRightStatements(value) {
                    return value.startsWith(
                               'opyright', 1) && // include copyright statements
                           !value.startsWith(
                               'opyright notice',
                               1) && // exclude lines from from license text
                           !value.startsWith('opyright and related rights', 1);
                  })
                  .filter(function removeDuplicates(
                      value, index,
                      list) { return index === 0 || value !== list[0]; });

          if (linesWithCopyright.length > 0) {
            moduleInfo.copyright =
                linesWithCopyright[0].replace(/\n/g, '. ').trim();
          }
  ...
  noticeFiles.forEach(function(filename) {
    var file = join(json.path, filename);
    /*istanbul ignore else*/
    if (lstatSync(file).isFile()) {
      moduleInfo.noticeFile =
          options.basePath ? relative(options.basePath, file) : file;
    }
  });
...
export function init(options, callback) {
  ...
  var opts = {dev : true, log : debugLog, depth : options.direct};
  ...
  read(options.start, opts, function(err, json) {
    var data = flatten({
      ...
      _args : options
        ...
        colorize = options.color, sorted = {}, filtered = {},
        exclude =
            options.exclude &&
    ...
    var colorizeString = function(string) {
      /*istanbul ignore next*/
      return colorize ? bold.red(string) : string;
    };
    ...
    if (exclude) {
      ...
      var invert = function(
          fn) { return function(spdx) { return !fn(spdx); }; };
      ...
      var invalidSPDXLicenses =
          exclude.map(transformBSD).filter(invert(spdxIsValid));
      ...
      Object.keys(sorted).forEach(function(item) {
        ...
        if (!licenses) {
          ...
          licenses.forEach(function(license) {
            /*istanbul ignore if - just for protection*/
            if (license.indexOf(UNKNOWN) >=
                0) { // necessary due to colorization
              filtered[item] = sorted[item];
            } else {
    ...
    Object.keys(restricted).forEach(function(item) {
      if (toCheckforFailOn.length > 0) {
        if (toCheckforFailOn.indexOf(restricted[item].licenses) > -1) {
          console.error('Found license defined by the --failOn flag: "' +
                        restricted[item].licenses + '". Exiting.');
          process.exit(1);
        }
      }
      if (toCheckforOnlyAllow.length > 0) {
        ...
        if (!good) {
          console.error(
              'Package "' + item + '" is licensed under "' +
...
export function print(sorted) { console.log(asTree(sorted)); }

export function asTree(sorted) { return treeify.asTree(sorted, true); }

export function asSummary(sorted) {
  var licenseCountObj = {};
  ...
  Object.keys(licenseCountObj).forEach(function(license) {
    licenceCountArray.push(
        {license : license, count : licenseCountObj[license]});
  });

  /*istanbul ignore next*/
  licenceCountArray.sort(function(a, b) { return b['count'] - a['count']; });
...
export function asCSV(sorted, customFormat, csvComponentPrefix) {
  ...
  if (customFormat && Object.keys(customFormat).length > 0) {
    ...
    ['"module name"', '"license"', '"repository"'].forEach(function(
        item) { textArr.push(item); });
    text.push(textArr.join(','));
  }

  Object.keys(sorted).forEach(function(key) {
    var module = sorted[key], line = '';
    lineArr = [];
 ...
 * @param  {JSON} customFormat The custom format with information about the
 *     needed keys.
 * @return {String}            The returning plain text.
 */
export function asMarkDown(sorted, customFormat) {
  ...
  if (customFormat && Object.keys(customFormat).length > 0) {
    Object.keys(sorted).forEach(function sortedCallback(sortedItem) {
      ...
      Object.keys(customFormat).forEach(function customCallback(customItem) {
        text.push('    - ' + customItem + ': ' +
                  sorted[sortedItem][customItem]);
      });
    });
    text = text.join('\n');
  } else {
    Object.keys(sorted).forEach(function(key) {
      ...
      text.push('[' + key + '](' + module.repository + ') - ' +
                module.licenses);
    });
    text = text.join('\n');
  }
...
export function parseJson(jsonPath) {
  if (typeof jsonPath !== 'string') {
    return new Error('did not specify a path');
  }

  var jsonFileContents = '', result = {};

  try {
    jsonFileContents = readFileSync(jsonPath, {encoding : 'utf8'});
...
export function asFiles(json, outDir) {
  sync(outDir);
  Object.keys(json).forEach(function(moduleName) {
    var licenseFile = json[moduleName].licenseFile, fileContents, outFileName,
        outPath, baseDir;

    if (licenseFile && existsSync(licenseFile)) {
      fileContents = readFileSync(licenseFile);
      outFileName = moduleName + "-LICENSE.txt";
      outPath = join(outDir, outFileName);
      baseDir = dirname(outPath);
      sync(baseDir);

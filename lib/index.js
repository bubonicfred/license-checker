...
var flatten = function(options) {
  const moduleInfo = {licenses : UNKNOWN}, json = options.deps;
  let data = options.data;
  const key = json.name + '@' + json.version, colorize = options.color,
        unknown = options.unknown;
  let readmeFile, licenseData, dirFiles, files = [], noticeFiles = [],
                                         licenseFile;
  ...
  const include = function(property) {
      ...
      if (index === 0) {
        ...
        if (include("licenseFile")) {
          moduleInfo.licenseFile =
              options.basePath ? path.relative(options.basePath, licenseFile)
                               : licenseFile;
        }
  ...
  noticeFiles.forEach(function(filename) {
    ...
    if (fs.lstatSync(file).isFile()) {
      moduleInfo.noticeFile =
          options.basePath ? path.relative(options.basePath, file) : file;
    }
  });
...
exports.init = function(options, callback) {
  ...
  const opts = {dev : true, log : debugLog, depth : options.direct};
  ...
  read(options.start, opts, function(err, json) {
    ...
    const colorizeString = function(string) {
      /*istanbul ignore next*/
    ...
    if (exclude) {
      const transformBSD = function(spdx) {
        return spdx === 'BSD'
      ...
      const spdxIsValid = function(spdx) { return spdxCorrect(spdx) === spdx; };

      const validSPDXLicenses = exclude.map(transformBSD).filter(spdxIsValid);
      const invalidSPDXLicenses =
    ...
    Object.keys(restricted).forEach(function(item) {
      if (toCheckforFailOn.length > 0) {
        if (toCheckforFailOn.indexOf(restricted[item].licenses) > -1) {
          throw new Error(`Found license defined by the --failOn flag: "${
      ...
      if (toCheckforOnlyAllow.length > 0) {
        ...
        if (!good) {
          throw new Error(`Package "${item}" is licensed under "${
              restricted[item]
                  .licenses}" which is not permitted by the --onlyAllow flag. Exiting.`);
        }
      }
    });
...
exports.print = function(sorted) { console.log(exports.asTree(sorted)); };
...
exports.asCSV = function(sorted, customFormat, csvComponentPrefix) {
  ...
  const prefix = csvComponentPrefix;
...
exports.asFiles = function(json, outDir) {
  ...
  Object.keys(json).forEach(function(moduleName) {
    ...
    let fileContents, outFileName, outPath, baseDir;

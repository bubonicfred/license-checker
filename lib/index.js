...
var flatten = (options) => {
    ...
    const include = (property) => (options.customFormat === undefined || options.customFormat[property] !== false);
    ...
    if (json.path && (!json.readme || json.readme.toLowerCase().includes('no readme data found'))) {
...
exports.init = function(options, callback) {
    ...
    read(options.start, opts, (err, json) => {
        ...
        const sorted = {};
        let filtered = {};
        const exclude = options.exclude && options.exclude.match(/([^\\\][^,]|\\,)+/g).map((license) => license.replace(/\\,/g, ',').replace(/^\s+|\s+$/g, ''));
        let inputError = null;
        ...
        Object.keys(data).sort().forEach((item) => {
            ...
            if (options.unknown && (data[item].licenses && data[item].licenses !== UNKNOWN) && data[item].licenses.includes('*')) {
                /*istanbul ignore if*/
                data[item].licenses = colorizeString(UNKNOWN);
            }
            /*istanbul ignore else*/
        ...
        if (exclude) {
            const transformBSD = (spdx) => spdx === 'BSD' ? '(0BSD OR BSD-2-Clause OR BSD-3-Clause OR BSD-4-Clause)' : spdx;

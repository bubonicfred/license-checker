...
import { existsSync, readFileSync, readdirSync, lstatSync, writeFileSync } from 'fs';
...
var flatten = function(options) {
        ...
        if (lstatSync(licenseFile).isFile()) {
            ...
            if (index === 0) {
                ...
                if (include("licenseFile")) {
                    moduleInfo.licenseFile = options.basePath ? relative(options.basePath, licenseFile) : licenseFile;
                }
    ...
    noticeFiles.forEach(function(filename) {
        ...
        if (lstatSync(file).isFile()) {
            moduleInfo.noticeFile = options.basePath ? relative(options.basePath, file) : file;
        }
    });
...
export function print(sorted) {
    console.log(asTree(sorted));
}

export function asTree(sorted) {
    return asTree(sorted, true);
}
...
export function asSummary(sorted) {
    ...
    return asTree(sortedLicenseCountObj, true);
}

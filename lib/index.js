...
exports.init = function (options, callback) {
  ...
  if (options.production || options.development) {
    ...
    read(options.start, opts, function(err, json) {
        ...
        const sorted = {};
        let filtered = {};
        const exclude = options.exclude && options.exclude.match(/([^\\\][^,]|\\,)+/g).map(function(license) {
    ...
    if (!Object.keys(sorted).length) {
      err = new Error("No packages found in this path..");
        ...
        if (exclude) {
            ...
            const invert = function(fn) { return function(spdx) { return !fn(spdx);};};
    ...
    if (options.excludePrivatePackages) {
      Object.keys(filtered).forEach(function (key) {
        ...
        Object.keys(restricted).forEach(function(item) {
            if (toCheckforFailOn.length > 0 && toCheckforFailOn.indexOf(restricted[item].licenses) > -1) {
            ...
            toCheckforOnlyAllow.forEach(function(k) {
                good = restricted[item].licenses.indexOf(k) === -1 && !good ? false : true;
            });
            if (!good) {

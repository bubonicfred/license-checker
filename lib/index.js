...
var flatten = (options) => {
  ...
  if (
    json.path &&
    (!json.readme || json.readme.toLowerCase().includes("no readme data found"))
  ) {
...
exports.init = function (options, callback) {
  ...
  read(options.start, opts, (err, json) => {
    ...
    if (exclude) {
      ...
      const invalidSPDXLicenses = exclude
        .map(transformBSD)
    ...
    Object.keys(restricted).forEach((item) => {
      if (
        toCheckforFailOn.length > 0 &&
        toCheckforFailOn.includes(restricted[item].licenses)
      ) {
        console.error(
          `Found license defined by the --failOn flag: "${restricted[item].licenses}". Exiting.`,
        );
        process.exit(1);
      }
      ...
      if (!good) {
        console.error(
          `Package "${item}" is licensed under "${restricted[item].licenses}" which is not permitted by the --onlyAllow flag. Exiting.`,
        );
        process.exit(1);
      }
    });

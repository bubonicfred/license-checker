...
describe("license files detector", function () {
  ...
  it("one license candidate", function () {
    assert.deepEqual(licenseFiles(["LICENSE", ".gitignore", "src"]), [
      "LICENSE",
    ]);
  });
  ...
  it("extensions have no effect", function () {
    assert.deepEqual(licenseFiles(["LICENCE.txt", ".gitignore", "src"]), [
      "LICENCE.txt",
    ]);
  });

  it("lower/upper case has no effect", function () {
    assert.deepEqual(licenseFiles(["LiCeNcE", ".gitignore", "src"]), [
      "LiCeNcE",
    ]);
  });
  ...
  it("LICENSE-MIT gets matched", function () {
    assert.deepEqual(
      licenseFiles(["LICENSE", ".gitignore", "LICENSE-MIT", "src"]),
      ["LICENSE", "LICENSE-MIT"],
    );
  });
  ...
  it("only the first LICENSE-* file gets matched", function () {
    assert.deepEqual(
      licenseFiles(["license-foobar.txt", ".gitignore", "LICENSE-MIT"]),
      ["license-foobar.txt"],
    );
  });
});

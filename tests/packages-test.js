...
describe("bin/license-checker", function() {
  ...
  it("should restrict the output to the provided packages", function() {
    ...
    var output = spawn("node",
                       [
                         path.join(__dirname, "../bin/license-checker"),
                         "--json", "--packages", restrictedPackages.join(";")
                       ],
                       {
                         cwd : path.join(__dirname, "../"),
                       });

    assert.deepEqual(Object.keys(JSON.parse(output.stdout.toString())),
                     restrictedPackages);
  });
  ...
  it("should exclude provided excludedPackages from the output", function() {
    ...
    var output =
        spawn("node",
              [
                path.join(__dirname, "../bin/license-checker"), "--json",
    ...
    excludedPackages.forEach(function(
        pkg) { assert.ok(!packages.includes(pkg)); });
  });

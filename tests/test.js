var assert = require('assert'), path = require('path'),
    checker = require('../lib/index'), args = require('../lib/args'),
    chalk = require('chalk'), fs = require('fs');

describe('main tests', function() {
  it('should load init',
     function() { assert.equal(typeof checker.init, 'function'); });

  it('should load print',
     function() { assert.equal(typeof checker.print, 'function'); });

  describe('should parse local with unknown', function() {
    var output;
    before(function(done) {
      this.timeout(5000);
      checker.init({start : path.join(__dirname, '../')},
                   function(err, sorted) {
                     output = sorted;
                     done();
                   });
    ...
    it('and convert to MarkDown', function() {
      ...
      assert.equal(
          '[@babel/code-frame@7.21.4](https://github.com/babel/babel) - MIT',
          str.split('\n')[0]);
    });
  });
  ...
  describe('should parse local with unknown and custom format', function() {
    ...
    before(function(done) {
      ...
      checker.init({start : path.join(__dirname, '../'), customFormat : format},
                   function(err, sorted) {
                     output = sorted;
                     done();
                   });
    ...
    it('and give us results', function() {
      ...
      assert.equal(output['abbrev@1.0.9'].description,
    ...
    it('and convert to CSV', function() {
      ...
      assert.equal('"module name","name","description","pewpew"',
                   str.split('\n')[0]);
      assert.equal(
          "@babel/code-frame", "@babel/code-frame",
          "Generate errors that contain a code frame that point to source locations.",
          '"<<Should Never be set>>"', str.split('\n')[1]);
    });
    ...
    it('and convert to CSV with component prefix', function() {
      ...
      assert.equal('"component","module name","name","description","pewpew"',
                   str.split('\n')[0]);
      assert.equal(
          '"main-module","@babel/code-frame@7.21.4","@babel/code-frame","Generate errors that contain a code frame that point to source locations.","<<Should Never be set>>"',
          str.split('\n')[1]);
    });
  ...
  describe('should parse local without unknown', function() {
    var output;
    before(function(done) {
      checker.init({start : path.join(__dirname, '../'), unknown : true},
                   function(err, sorted) {
                     output = sorted;
                     done();
                   });
  ...
  function parseAndExclude(parsePath, licenses, result) {
    return function(done) {
      checker.init(
          {start : path.join(__dirname, parsePath), exclude : licenses},
  ...
  describe('should parse local with unknown and excludes', function() {
    ...
    it('should exclude MIT and ISC licensed modules from results', function() {
      ...
      Object.keys(output).forEach(function(item) {
        if (output[item].licenses && (output[item].licenses === "MIT" ||
                                      output[item].licenses === "ISC"))
          excluded = false;
      });
      assert.ok(excluded);
    });
  ...
  describe('should parse local with excludes containing commas', function() {
    var result = {};
    before(parseAndExclude('./fixtures/excludeWithComma',
                           "Apache License\\, Version 2.0", result));

    it('should exclude a license with a comma from the list', function() {
      var excluded = true;
      var output = result.output;
      Object.keys(output).forEach(function(item) {
        if (output[item].licenses &&
            output[item].licenses === "Apache License, Version 2.0")
          excluded = false;
      });
      assert.ok(excluded);
    });
  ...
  describe('should parse local with Public Domain excludes', function() {
    var result = {};
    before(parseAndExclude('./fixtures/excludePublicDomain', "Public Domain",
                           result));
  ...
  describe('should not exclude Custom if not specified in excludes',
           function() {
             var result = {};
             before(parseAndExclude('./fixtures/custom-license-file', "MIT",
                                    result));
             ...
             it('should exclude Public Domain', function() {
               ...
               Object.keys(output).forEach(function(item) {
                 if (output[item].licenses &&
                     output[item].licenses === "Custom: MY-LICENSE.md")
                   excluded = false;
               });
               assert.ok(!excluded);
             });
  ...
  function parseAndFailOn(key, parsePath, licenses, result) {
    return function(done) {
      ...
      var config = {start : path.join(__dirname, parsePath)};
      config[key] = licenses;
  ...
  describe('should exit on given list of onlyAllow licenses', function() {
    ...
    it('should exit on non MIT and ISC licensed modules from results',
       function() { assert.equal(result.exitCode, 1); });
  });
  ...
  describe('should not exit on complete list', function() {
    ...
    it('should not exist if list is complete',
       function() { assert.equal(result.exitCode, 0); });
  });

  describe('should exit on given list of failOn licenses', function() {
    var result = {};
    before(
        parseAndFailOn('failOn', '../', "Apache License, Version 2.0", result));

    it('should exit on Apache License, Version 2.0 licensed modules from results',
       function() { assert.equal(result.exitCode, 1); });
  });
  ...
  describe('should parse local and handle private modules', function() {
    ...
    it('should reconise private modules', function() {
      ...
      Object.keys(output).forEach(function(item) {
        if (output[item].licenses &&
            output[item].licenses.indexOf("UNLICENSED") >= 0) {
          privateModule = true;
        }
      });
  ...
  describe('should treat license file over custom urls', function() {
    it('should recognise a custom license at a url', function(done) {
      checker.init({start : path.join(__dirname, '../node_modules/locale')},
                   function(err, output) {
  ...
  describe('should treat URLs as custom licenses', function() {
    ...
    it('should recognise a custom license at a url', function() {
      ...
      Object.keys(output).forEach(function(item) {
        if (output[item].licenses &&
            (output[item].licenses ===
             "Custom: http://example.com/dummy-license"))
  ...
  describe('error handler', function() {
    it('should init without errors', function(done) {
      checker.init({start : path.join(__dirname, '../'), development : true},
                   function(err) {
    ...
    it('should init with errors (npm packages not found)', function(done) {
      checker.init({start : 'C:\\'}, function(err) {
  ...
  describe('should parse with args', function() {
    ...
    it('should handle direct true', function() {
      var result = args.defaults(
          {direct : true, start : path.resolve(path.join(__dirname, '../'))});
  ...
  describe('custom formats', function() {
    it('should create a custom format using customFormat successfully',
       function(done) {
         checker.init({
    ...
    it('should create a custom format using customPath', function(done) {
      ...
      checker.init(args, function(err, filtered) {
        var customFormatContent = fs.readFileSync(
            path.join(__dirname, './../customFormatExample.json'), 'utf8');
  ...
  describe('should output the module location', function() {
    it('as absolute path', function(done) {
      checker.init(
          {start : path.join(__dirname, '../')}, function(err, output) {
  ...
  describe('should output the location of the license files', function() {
    it('as absolute paths', function(done) {
      checker.init({start : path.join(__dirname, '../')}, function(err,
                                                                   output) {
        Object.keys(output)
            .map(function(key) { return output[key]; })
            .filter(function(dep) { return dep.licenseFile !== undefined; })
    ...
    it('as relative paths when using relativeLicensePath', function(done) {
      checker.init(
          {start : path.join(__dirname, '../'), relativeLicensePath : true},
          function(err, filtered) {
            Object.keys(filtered)
                .map(function(key) { return filtered[key]; })
                .filter(function(dep) { return dep.licenseFile !== undefined; })
                .forEach(function(dep) {
  ...
  describe('handle copytight statement', function() {
    it('should output copyright statements when configured in custom format',
       function(done) {
         checker.init(
             {
               start : path.join(__dirname, '../'),
             ...
             function(err, output) {
               assert(
                   Object.prototype.hasOwnProperty.call(output, 'abbrev@1.0.9'),
                   'Check if the expected package still exists.');
               assert.equal(
                   output['abbrev@1.0.9'].copyright,
  ...
  describe(
      'should only list UNKNOWN or guessed licenses successful', function() {
        var output;
        before(function(done) {
          checker.init(
              {start : path.join(__dirname, '../'), onlyunknown : true},
              function(err, sorted) {
                output = sorted;
                done();
              });
        ...
        it('so we check if there is no license with a star or UNKNOWN found',
           function() {
             ...
             Object.keys(output).forEach(function(item) {
               if (output[item].licenses &&
                   output[item].licenses.indexOf('UNKNOWN') !== -1) {
                 // Okay
               } else if (output[item].licenses &&
                          output[item].licenses.indexOf('*') !== -1) {
                 // Okay
               } else {
  ...
  describe(
      ...
      function() {
        var output;
        before(function(done) {
          checker.init({start : path.join(__dirname, '../'), production : true},
                       function(err, sorted) {
                         output = sorted;
                         done();
                       });
  ...
  describe('should export', function() {
    ...
    it('as files', function() {
      var out = path.join(require('os').tmpdir(), 'lc'), files;
      checker.asFiles({
        foo : {
          licenses : 'MIT',

...
describe("main tests", function () {
  ...
  describe("should parse local with unknown and custom format", function () {
    ...
    before(function (done) {
      ...
      checker.init(
        { start: path.join(__dirname, "../"), customFormat: format },
        function (err, sorted) {
          output = sorted;
          done();
        },
    ...
    it("and give us results", function () {
      ...
      assert.equal(
        output["abbrev@1.0.9"].description,
  ...
  describe("should parse local without unknown", function () {
    ...
    before(function (done) {
      checker.init(
        { start: path.join(__dirname, "../"), unknown: true },
        function (err, sorted) {
          output = sorted;
          done();
        },
    ...
    it("should exclude MIT and ISC licensed modules from results", function () {
      ...
      Object.keys(output).forEach(function (item) {
        if (
          ...
          (output[item].licenses === "MIT" || output[item].licenses === "ISC")
        )
          excluded = false;
      });
      assert.ok(excluded);
  ...
  describe("should treat URLs as custom licenses", function () {
    ...
    it("should recognise a custom license at a url", function () {
      ...
      Object.keys(output).forEach(function (item) {
        if (
          output[item].licenses &&
          output[item].licenses === "Custom: http://example.com/dummy-license"
        )
  ...
  describe("error handler", function () {
    it("should init without errors", function (done) {
      checker.init(
        { start: path.join(__dirname, "../"), development: true },
        function (err) {
  ...
  describe("should only list UNKNOWN or guessed licenses with errors (argument missing)", function () {
    ...
    before(function (done) {
      checker.init(
        { start: path.join(__dirname, "../"), production: true },
        function (err, sorted) {
          output = sorted;
          done();
        },
